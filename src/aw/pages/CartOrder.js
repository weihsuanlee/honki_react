...
function CartOrder(props) {
  ...
  const getDataFromServer = async () => {
    ...
    const response = await fetch(
      'https://cors-anywhere.herokuapp.com/https://honki-books.herokuapp.com/cart/order_detail',
      {
        credentials: 'include',
        method: 'get',
        // headers: new Headers({
  ...
  const getDataFromServer2 = async () => {
    ...
    // 注意header資料格式要設定，伺服器才知道是json格式
    const response = await fetch(
      'https://cors-anywhere.herokuapp.com/https://honki-books.herokuapp.com/cart/order_detail_input',
      {
        credentials: 'include',
        method: 'get',
        // headers: new Headers({
